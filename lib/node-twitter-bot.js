...
NodeTwitterBot.prototype = {
  ...
  followByUser: function(screen_name, options) {
    ...
    var pipeline = this.plumber.pipefy()
      .listenTo('followByUser')
      .pipe(this.twitter_api.getUserFollowers, null, null, 'twit', 'setDep', 'next')
      .pipe(this.twitter_api.followingByUsersList, 'users_list', 'twit', 'next');

    pipeline.trigger('followByUser', screen_name, options);
  },
